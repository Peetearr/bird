...
def control(err, derr, kp = 1, kd = .01, A_0 = 1.5):
...
for body_id in range(m.nbody):
    # mass[body_id] - масса тела (без инерции)
    # inertia[body_id] - тензор инерции (3x3 матрица)
...
while True:
    ...
    w = 32
    A = control(err, d_err)
    tail_pitch = control(pitch_err, pitch_d_err, A_0=0, kp=1, kd=.3)
